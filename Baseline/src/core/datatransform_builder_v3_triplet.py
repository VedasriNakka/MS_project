...
python src/core/datatransform_builder_v3.py --transform_type randomcrop224,affine
python src/core/datatransform_builder_v3_triplet.py --transform_type randomcrop224 --loss_fn triplet
...
def main(args):
    ...
    if (
        ...
        output_dir = os.path.join(
            "/home/vedasri/Baseline_V2/augmentation_visuals_triplet",
            phase,
            args.transform_type,
        )
        ...
        for index, (anchor, positive, negative, labels) in enumerate(dataloaders[phase]):
            ...
            #anchor = [img["anchor"] for img in anchor]
            ...
            print(torch.mean(anchor))

            # Concatenate images along the width (dimension 3)
            triplet = torch.cat((anchor, positive, negative), dim=2)  # Concatenate along the width dimension

            save_image_batch(
                triplet, f"{index}_" + transform_name + "_" + phase, output_dir
            )
            if index == 5:
                break


    elif args.transform_type == "randomcrop224,gaussianblur,gray,invert":
    ...
    elif args.transform_type == "randomcrop224,morpho_dilation":
...
if __name__ == "__main__":
    ...
    ap.add_argument("--loss_fn", help="loss function", default="triplet", type=str)
